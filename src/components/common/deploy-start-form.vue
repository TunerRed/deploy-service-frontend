<template>
  <el-form v-model="deployForm" :inline="line" ref="deployForm">
    <el-form-item label="可选服务器">
      <el-select v-model="deployForm.serverIP" placeholder="选择要部署的服务器">
        <el-option v-for="(item, index) in deployForm.serverIPList" :key="index" :label="item" :value="item"></el-option>
      </el-select>
    </el-form-item>
    <el-form-item  prop="phoneNumber" label="短信通知电话">
      <el-input v-model="deployForm.phoneNumber" placeholder="请输入电话" class="phone-input" maxlength="11"></el-input>
    </el-form-item>
